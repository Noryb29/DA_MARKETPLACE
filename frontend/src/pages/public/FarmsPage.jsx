...
const FarmCard = ({ farm, onViewClick, index }) => {
  ...
  const location = [farm.barangay, farm.municipality, farm.province].filter(Boolean).join(', ')
      ...
      <div className="p-4">
        ...
        {location && (
          <div className="flex items-start gap-1.5 mb-3">
            ...
            <span className="text-xs text-gray-500 line-clamp-2">{location}</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 flex-wrap pt-2 border-t border-gray-100">
          ...
          {docsCount > 0 && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs font-semibold text-purple-700">{docsCount}</span>
            </div>
          )}
        </div>

        <button className="w-full mt-4 py-2.5 bg-gray-50 group-hover:bg-green-50 text-gray-700 group-hover:text-green-700 text-xs font-semibold rounded-lg transition-colors flex items-center justify-center gap-1">
          View Farm
