...
import { MapPin, X, Image, FileText, Upload, Search, Loader } from 'lucide-react';
...
const MapSelector = ({ onLocationSelect, selectedCoords }) => {
  ...
  useEffect(() => {
    if (selectedCoords && selectedCoords.includes(', ')) {
      ...
      <div className="relative">
        <form onSubmit={handleSearch} className="relative">
          <input
            ...
            onFocus={() => searchQuery.length > 2 && searchLocation(searchQuery)}
          />
          <button
            type="submit"
