...
const FarmerChatPage = () => {
  ...
  const { logout } = useFarmerAuthStore()
  const [messageInput, setMessageInput] = useState('')
  const [sending, setSending] = useState(false)
  const [showConversations, setShowConversations] = useState(true)
  const messagesEndRef = useRef(null)
  const farmerId = JSON.parse(localStorage.getItem('farmer'))?.user_id
  const role = 'farmer'

  useEffect(() => {
    if (farmerId) {
      initSocket(farmerId, role)
    }
    getConversations()
    return () => {
                    ...
                    <div key={conv.conversation_id} onClick={() => { setCurrentConversation(conv.conversation_id); if (window.innerWidth < 768) setShowConversations(false) }}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-sm text-gray-900">{conv.firstname} {conv.lastname}</p>
            ...
            {currentConversation ? (
              <>
                <div className="p-4 bg-white border-b border-gray-200 flex items-center gap-3">
                  ...
                  {selectedConv && (
                    <>
                      ...
                      <div>
                        <h3 className="font-semibold text-gray-900">{selectedConv.firstname} {selectedConv.lastname}</h3>
                ...
                <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
                  {messages.map((msg) => {
                    const isOwn = msg.sender_type === 'farmer'
                    return (
