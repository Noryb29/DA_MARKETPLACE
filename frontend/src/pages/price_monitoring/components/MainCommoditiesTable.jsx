...
const MainCommodityTable = ({ 
  ...
  const grouped = useMemo(() => {
    return dataToDisplay.reduce((acc, v) => {
      ...
      <div className="ctbl-wrap flex flex-col gap-4 w-full">
        ...
        <div className="flex items-center justify-between px-1">
          <span className="text-xs text-slate-500 font-medium">
            {dataToDisplay.length} commodit{dataToDisplay.length !== 1 ? "ies" : "y"}
            {dataToDisplay.length > 0 && (
              <span className="text-slate-400 ml-3">
                (showing {startIndex + 1}–{Math.min(endIndex, dataToDisplay.length)} per page)
              </span>
            )}
          </span>
        ...
        {/* Pagination Controls */}
        {dataToDisplay.length > 0 && totalPages > 1 && (
          <div className="flex items-center justify-between px-1">
            <span className="text-xs text-slate-500 font-medium">
